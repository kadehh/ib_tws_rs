...
use crate::{
    ...
    message::{
        ...
        request::{
            Handshake, ReqAccountSummary, ReqAccountUpdates, ReqContractDetails, ReqHistoricalData,
            ReqMarketDataType, ReqMktData, ReqMktDepth, ReqMktDepthExchanges, ReqTickByTickData,
            SetServerLogLevel, StartApi, ReqRealtimeBars, ReqHistoricalTicks,
        },
        response::{
            AccountSummaryMsg, AccountUpdatesMsg, HandshakeAck, HistoricalDataMsg,
            MktDepthExchangesMsg, RealTimeBarsMsg, HistoricalTicksMsg, HistoricalTickBidAskMsg, HistoricalTickLastMsg,
        },
        Request, Response,
    },
    Error,
...
impl AsyncClient {
    ...
    #[instrument(skip(self))]
    pub async fn request_account_update(
        &self,
        message: ReqAccountUpdates,
    ) -> Result<impl Stream<Item = Result<AccountUpdatesMsg, Error>> + '_, Error> {
        let request_id = self.send(Request::ReqAccountUpdates(message)).await?;

        Ok(self
            .response_stream_by_id(Some(request_id))
            .take_while(|response| {
                let is_end = matches!(response, Response::AccountUpdatesEndMsg(_));
                async move { !is_end }
            })
            .filter_map(|response| async move {
                match response {
                    Response::ErrMsgMsg(err) => Some(Err(Error::ApiError(err))),
                    Response::AccountUpdatesMsg(msg) => Some(Ok(msg)),
                    _ => None,
                }
            }))
    }
    ...
    pub async fn request_historical_ticks(
        &self,
        message: ReqHistoricalTicks,
    ) -> Result<HistoricalTicksMsg, Error> {
        let request_id = self.send(Request::ReqHistoricalTicks(message)).await?;
        ...
        Box::pin(
            self.response_stream_by_id(Some(request_id))
                .filter_map(|response| async move {
                    match response {
                        Response::ErrMsgMsg(err) => Some(Err(Error::ApiError(err))),
                        Response::HistoricalTicksMsg(msg) => Some(Ok(msg)),
                        _ => None,
                    }
                }),
        )
    ...
    // request_history_ticks last
    #[instrument(skip(self))]
    pub async fn request_historical_ticks_last(
        &self,
        message: ReqHistoricalTicks,
    ) -> Result<HistoricalTickLastMsg, Error> {
        let request_id = self.send(Request::ReqHistoricalTicks(message)).await?;
        ...
        Box::pin(
            self.response_stream_by_id(Some(request_id))
                .filter_map(|response| async move {
                    match response {
                        Response::ErrMsgMsg(err) => Some(Err(Error::ApiError(err))),
                        Response::HistoricalTickLastMsg(msg) => Some(Ok(msg)),
                        _ => None,
                    }
                }),
        )
    ...
    // request_history_ticks bid_ask
    #[instrument(skip(self))]
    pub async fn request_historical_ticks_bid_ask(
        &self,
        message: ReqHistoricalTicks,
    ) -> Result<HistoricalTickBidAskMsg, Error> {
        let request_id = self.send(Request::ReqHistoricalTicks(message)).await?;
        ...
        Box::pin(
            self.response_stream_by_id(Some(request_id))
                .filter_map(|response| async move {
                    match response {
                        Response::ErrMsgMsg(err) => Some(Err(Error::ApiError(err))),
                        Response::HistoricalTickBidAskMsg(msg) => Some(Ok(msg)),
                        _ => None,
                    }
                }),
        )
    ...
    #[instrument(skip(self))]
    pub async fn request_real_time_bars(
        &self,
        message: ReqRealtimeBars, // Assuming ReqRealTimeBars is your request type
    ) -> Result<impl Stream<Item = Result<RealTimeBarsMsg, Error>> + '_, Error> {
        let request_id = self.send(Request::ReqRealtimeBars(message)).await?;
        ...
        Ok(self
            ...
            .filter_map(|response| async move {
                match response {
                    Response::ErrMsgMsg(err) => Some(Err(Error::ApiError(err))),
                    Response::RealTimeBarsMsg(msg) => Some(Ok(msg)),
                    _ => None,
                }
            }))
    }
